// =============================================================================
// Task Operations - File I/O and helper functions
// =============================================================================

import { existsSync } from "node:fs";
import YAML from "yaml";
import { type Task, type TaskStatus, type TasksFile, validateTasksFile } from "./schema";
...
export function isCheckpointTask(task: Task): boolean {
...
export async function primeTasks(
  ...
  askQuestionFn: (
    ...
    options?: {
      ...
      action?: { type: string; onYes?: string; onNo?: string };
    }
  ) => Promise<string>
): Promise<number> {
  const completedIds = new Set<string>();
  ...
  for (const task of checkpointTasks) {
    ...
    await askQuestionFn("orchestrator", question, {
      taskId: task.id,
      questionType: "yes_no",
      action: {
        type: "set_status",
